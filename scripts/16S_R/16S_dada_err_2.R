...
run.metaD<-run.metaD[(!run.metaD$sampleNames=="YoTB_2"),] # had no reads that passed filter...
...
filt_out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(250,250), #trimLeft=c(19,20),
                     maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE,
                     compress=TRUE, multithread=12) # On Windows set multithread=FALSE
...
#write.csv(filt_out, file="/projects/ps-shurinlab/users/cbwall/YoZoop_microbiome_21.22/output/filt_out.csv")
...
# pool= TRUE and pseudo, we will use psuedo here
# TRUE takes a lot more time and memory
dadaFs <-dada(derepFs, err=errF, multithread=12, pool="pseudo")
saveRDS(dadaFs, "output/dadaFs.rds")
#dadaFs<- readRDS("output/dadaFs.rds")

dadaRs <-dada(derepRs, err=errF, multithread=12, pool="pseudo")
...
sum(seqtab.nochim)/sum(seqtab) # 98% of samples kept, chimeras ~ 3% of merged reads
...
taxa <- assignTaxonomy(seqtab.nochim, "/projects/ps-shurinlab/Databases/SILVA/silva_nr99_v138.1_train_set.fa.gz", multithread=12)
